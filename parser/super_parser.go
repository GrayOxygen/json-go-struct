...
var (
	...
	nestStructStr = ""
	structName    = "StructName" //输出的struct名称(顶级)
...
func adjustDefineInfo(ct string) string {
	...
	for index := lineCount - 1; index < len(scanner); index++ { //不管第一行和最后一行
		...
		if strings.TrimSpace(scanner[index]) != "" { //去掉json描述，更改类型和变量名的位置
			if strings.Contains(scanner[index], "obj-unique-by-davidwang") { //存在对象类型的不用再颠倒了
			...
			final_res += "	" + strings.TrimSpace(arrays[1]) + " " + strings.TrimSpace(arrays[0]) + "\n"
		}
	}
	return final_res
...
func genOneProtoNode(node *tree.TreeNode) error {
	...
	for _, c := range objs { //替换id为规范属性声明
		if c.Type == enums.STRUCT {
			temp = strings.Replace(temp, c.Id, c.Name+" "+util.LowerFirstRune(c.Name)+" obj-unique-by-davidwang ", -1)
		}
		if c.Type == enums.ARRAY {
			temp = strings.Replace(temp, c.Id, " repeated "+c.Name+" "+util.LowerFirstRune(c.Name)+" obj-unique-by-davidwang ", -1)
		}
	}
	//不是自己直接儿子进行忽略
	objs = tree.GetSonChildrenObjs(node)
	...
	for index := lineCount - 1; index < len(scanner)-1; index++ { //不管第一行和最后一行
		if strings.Contains(scanner[index], "obj-unique-by-davidwang") {
			scanner[index] = strings.Replace(scanner[index], "obj-unique-by-davidwang", " \n", -1)
